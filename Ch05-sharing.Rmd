...
of ggplots and options with class animint), then printing it on the R command line compiles that animint to a
temporary directory. Internally, R calls the `print.animint` S3 method, which
calls `animint2dir` to compile it to a new temporary directory on your personal
computer.
...
I can view it on http://members.cbio.mines-paristech.fr/~thocking/WorldBank-facets/
