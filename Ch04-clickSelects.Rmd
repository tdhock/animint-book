...
point? Hint: you need to add an `aes` to the `geom_text`.
...
**Exercise:** Re-make the data viz above so that clicking a text label
  removes that country from the selection set. Hint: you just need to
  add one `aes` to the `geom_text`.
...
```{r Ch04-viz-facets}
viz.facets <- list(scatter=scatterTS)
