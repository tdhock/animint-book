...
As of release August 2022, it is also possible to specify the opacity
and color for objects which are not currently selected, using the
`alpha_off` and `color_off` parameters. Users can specify these
parameters to freely create a different appearance for selected and
un-selected items, instead of being forced to rely on the behavior
described above. For more information, see the discussion and example
in [Chapter 6, section Specifying how selection state is
