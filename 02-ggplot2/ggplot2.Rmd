...
ggplots as web pages. This ggplot can be rendered with animint, by
first assigning the ggplot to a named element of a list, and then
giving that list the class `"animint"`.
...
ggplot `scatter` to a named element of the `scatter.viz` list, and the
...
`animint2dir` function, which converts the animint `scatter.viz` list to
