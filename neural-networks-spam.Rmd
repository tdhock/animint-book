...
bayes <- function(DT)DT[, (V1>0.45 & V1<0.9 & V2<0.8)|(V1<0.3 & V2>0.4)]
...
contour.list <- contourLines(
  V1.seq, V2.seq, 
  matrix(grid.dt$bayes.num, length(V1.seq), length(V2.seq), byrow=TRUE),
  levels=0)
contour.dt <- data.table(contour.i=seq_along(contour.list))[, {
...
min.dt <- loss.tall[, .SD[which.min(value)], by=.(set.name, variable)]
ggplot()+
