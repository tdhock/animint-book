...
viz.simple <- list(
  input=ggplot()+
    ...
    geom_vline(aes(xintercept=boundary),
               color="grey50",
               data=x1.boundaries)+
    geom_point(aes(x1, x2, color=y, clickSelects=data.i),
               size=4,
               alpha=0.7,
               data=YVAR(data.dt, "x2"))+
    geom_line(aes(x1, f),
              data=YVAR(scores, "f"))+
    geom_point(aes(x1, f.noise, color=y, clickSelects=data.i),
               size=4,
               alpha=0.7,
               data=YVAR(data.dt, "f")),
  square=ggplot()+
    ...
    geom_vline(aes(xintercept=boundary), color="grey50",
               data=x1sq.boundary)+
    geom_point(aes(x1.sq, x2.sq, color=y, clickSelects=data.i),
               size=4,
               alpha=0.7,
               data=YVAR(data.dt, "x2.sq"))+
    geom_line(aes(x1.sq, f),
              data=YVAR(scores, "f"))+
    geom_point(aes(x1.sq, f.noise, color=y, clickSelects=data.i),
               size=4,
               alpha=0.7,
               data=YVAR(data.dt, "f")))
...
ggplot()+
  geom_point(aes(x1.sq, x2.sq, color=pred.y, fill=y),
             shape=21,
             size=4,
             stroke=2,
             data=train.dt)
...
ggplot()+
  ...
  geom_point(aes(x1.sq, x2.sq, fill=y, color=train.error),
             shape=21,
             stroke=1,
             size=4,
             data=train.dt)+
...
ggplot()+
  scale_linetype_manual(values=c(margin="dashed", decision="solid"))+
  geom_vline(aes(xintercept=boundary), color="violet",
             data=x1sq.boundary)+
  geom_abline(aes(slope=slope, intercept=intercept, linetype=boundary),
              size=1,
              data=abline.dt)+
...
for(cost.param in 10^seq(-1, 2, by=cost.by)){
  for(degree.num in c(1, 2, 3, 4)){
    ...
    boundaries <- getBoundaryDF(grid.dt$pred.f)
    inputBoundaries.list[[paste(cost.param, degree.num)]] <- data.table(
...
viz.poly <- list(
  selectModel=ggplot()+
    ...
    ##           data=setErrors[cost.param==min(cost.param),])+
    ## guides(color="none")+
    geom_tile(aes(log10(cost.param), degree.num, fill=errors,
  ...
  inputSpace=ggplot()+
    ...
    geom_path(aes(x1, x2,
                  group=path.i,
                  linetype=boundary,
                  showSelected3=degree.num,
                  showSelected=cost.param),
              color="black",
              data=inputBoundaries)+
    geom_point(aes(x1, x2, color=y, fill=status,
                   showSelected=cost.param,
                   showSelected5=degree.num),
               size=3,
               data=predictions))
structure(viz.poly, class="animint")
...
In the previous section we fit an SVM with a polynomial kernel in the
original feature space. In this section we use the RBF kernel in the
original space.
