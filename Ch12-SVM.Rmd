...
function of `x1.sq`, and ignores `x2.sq`. 
...
In particular, we assume that the label `y` is negative (-1) if `x1.sq
< 1`, and positive (1) otherwise.
...
ggplot()+
  geom_point(aes(x1, f.noise),
             data=data.dt)
ggplot()+
  geom_point(aes(x1.sq, f.noise),
             data=data.dt)
