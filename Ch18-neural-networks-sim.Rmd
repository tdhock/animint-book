...
out.list <- list(
  ...
  min=min.dt,
  pred=pred.dt,
  contours=iteration.contours,
  errors=err.dt)
out.dir <- "Ch18-neural-networks-sim-data"
dir.create(out.dir)
for(data.name in names(out.list)){
  out.path <- file.path(out.dir, paste0(data.name, ".csv"))
  fwrite(out.list[[data.name]], out.path)
}
animint(
