...
We begin by loading the `montreal.bikes` data set and examining the
accidents data table.

```{r accidents}
library(animint2)
data(montreal.bikes)
## Tell R on windows that these factor levels are UTF-8 encoded.
if("Brébeuf" %in% montreal.bikes$counter.counts$location){
...
```{r timeRange}
accidents.dt[, date.POSIXct := suppressWarnings(strptime(date.str, "%Y-%m-%d"))]
...
period, so we first create a data table for each month below.
...
months[, next01.POSIXct := suppressWarnings(strptime(next01.str, "%Y-%m-%d"))]
...
animint(
  regression=ggplot()+
    theme_bw()+
    ggtitle("Accidents grows with number of cyclists")+
    geom_line(aes(
    ...
    geom_point(aes(
      ...
      alpha=0.75,
      data=scatter.max)+
    ylab("Total number of bike accidents in all Montreal locations")+
    xlab("Total number of cyclists counted in all Montreal locations"),
  timeSeries=ggplot()+
    theme_bw()+
