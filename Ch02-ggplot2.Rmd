...
* State the advantages of the grammar of graphics relative to previous
  plotting systems
* Install the animint2 R package from GitHub
...
## Installing animint2 {#installing}

The `animint2` R package is hosted on GitHub, so you can install
`animint2` by using the `devtools` package. To make sure that `animint2`
is installed, open R and execute the following code.

```{r}
if(!require(animint2)){
  install.packages("devtools")
  devtools::install_github("tdhock/animint2", upgrade_dependencies=FALSE)
}
```
...
**Exercise**: try changing the `region` legend to an `income`
  ...
  details](Ch17-appendix.html#manual-color-legends).
