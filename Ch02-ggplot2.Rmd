...
variables like `income` are best shown using point `color` or `fill`.
...
consider the following sketch adds a `geom_path` to the previous
data visualization.
...
viz.two.layers <- list(scatter=two.layers)
...
**Exercise**: try changing the `region` legend to an `income`
  ...
  [read the appendix for more details](ChXX-appendix.html#manual-color-legends).
...
Like usual, attaching the `"animint"` class to this list 
renders the data viz. Since the list contains two ggplots, animint
...
**Exercise**: make a data viz with three plots, by assigning a new
  ggplot to a named element of the `viz` list. For example, you could
  add a time series of another data variable such as `life.expectancy`
  or `population`.
