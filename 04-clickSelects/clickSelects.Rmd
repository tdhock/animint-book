...
either single or multiple selection variables. We also explained how
the `first` option can be used to specify the selected values that are
used when the animint is first rendered.
...
* Add `aes(size=population)` to the World Bank scatterplot. Is the
  size legend interactive? Why?
