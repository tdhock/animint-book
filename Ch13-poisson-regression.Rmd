...
viz.log.loss$loss <- ggplot()+
  ...
  geom_line(aes(log(poisson.mean), loss, showSelected=count),
            data=addX(finite.loss, "log(poisson mean)"))+
  geom_point(aes(log(count), loss, showSelected=count),
             data=addX(loss.min, "log(poisson mean)"))+
  geom_tallrect(aes(xmin=ifelse(min < 0, -Inf, log(min)), xmax=log(max),
